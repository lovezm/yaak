...
- [x] 检查请求标签当前渲染链与可插入切换的位置
- [x] 实现请求数据/完整报文切换，并补齐完整报文拼接逻辑
- [x] 更新任务记录与 lessons，并完成前端校验

## Review

- 已在响应面板的“请求”标签内新增 `请求数据 / 完整` 切换，默认仍保持原来的请求体视图
- 切到“完整”后，会拼出接近原始报文的整段文本：请求行、请求头、空行、请求体、响应行、响应头、空行、响应体
- 请求行会优先使用当前响应对应的请求方法与实际 URL，并带上协商后的 HTTP 版本；响应行会显示状态码和状态文本
- 若响应体看起来是二进制，会在完整视图里明确省略；若正文正在加载或加载失败，也会在整段文本里给出可见提示
- 完整视图额外提供了一键复制，方便直接整段导出
- 已把完整报文视图改成分段着色：请求行、请求头、请求体、响应行、响应头、响应体颜色会明显区分，便于快速扫读
- 已按结构化正文继续细分完整报文视图：`JSON` 正文会区分键、字符串、数字、布尔值和标点，`HTML` 正文会把标签与正文文本拆开着色
- 已把完整报文视图和代码生成视图改回可自由选中文本，同时保留复制按钮；正文内容可直接拖选复制，行号仍保持不可选，避免一并带出
- 前端校验通过：`npm run --workspace src-web lint`
